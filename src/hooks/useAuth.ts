...
export const useAuth = () => {
    ...
    try {
      ...
      if (token && userData) {
        ...
        if (response.data.success) {
          ...
          // AUTO SET ROLE IF NOT SET
          if (!savedRole) {
            const firstRole = Object.keys(userFromServer.roles)
              .find(r => userFromServer.roles[r] === true);
  ...
  const login = (userData, token) => {
    ...
    const firstRole = Object.keys(userData.roles)
      .find(r => userData.roles[r] === true);
