...
const ReviewerDashboard: React.FC = () => {
  ...
  const loadDashboard = async () => {
    ...
    try {
      ...
      if (reviewsRes.data?.success) {
        setAssignments(reviewsRes.data.data.assignments);
      }
    } catch (error: any) {
  ...
  const handleViewManuscript = async (assignment: ReviewAssignment) => {
    try {
      ...
      if (res.data?.success) {
        setViewingManuscript(res.data.data.manuscript);
        setShowReviewForm(false);
      }
    } catch (error: any) {
      setMessage({ type: 'error', text: error.response?.data?.message || 'Failed to load manuscript' });
    }
  };
                      ...
                      <button
                        onClick={async () => {
                          ...
                          try {
                            ...
                            if (res.data?.success) {
                              setFullReviewDetails(res.data.data.review);
                            }
                          } catch (error) {
